...
void watplot::plot(wavearray<double> &td, char* opt, int col, 
                   double t1, double t2, bool fft, float f1, float f2, bool psd, float t3) {
//
  ...
  if(w.isWDM()) {
    ...
    for(int i=it1; i<it2; ++i){
      if(i){
         ...
         v = procOpt(mode%3, A00*map00[M], A90*map90[M]); //first half-band
...
void watplot::plot(netcluster* pwc, int cid, int nifo, char type, int irate, char* opt, int pal) {
//
...
//
  ...
  gr->SetTitle(title);
  gr->GetHistogram()->SetStats(kFALSE);
  gr->GetHistogram()->SetTitleFont(12);
  ...
  gr->GetYaxis()->SetTitle("Frequency^{-8/3}");
...
void 
watplot::goptions(char* opt, int col, double t1, double t2, bool fft, float f1, float f2, bool psd, float t3) {
//
...
//
  ...
  if(x.size()==0)
    throw watplotException(kBreak,"watplot::operator(>>) ", "input array with size=0");
  ...
  graph.plot(x, const_cast<char*>(opt.Data()), graph.col+graph.ncol, t1, t2, graph.fft, f1, f2, graph.psd, graph.t3);
