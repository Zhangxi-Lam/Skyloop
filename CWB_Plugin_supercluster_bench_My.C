...
long subNetCut(network* net, int lag, float snc, TH2F* hist, struct test *test_s);
...
void alloc_test(struct test *test_s, int eTDDim);
void cleanup_test(struct test *test_s);
...
// This plugin implements the standard supercluster stage & use a local implementation of subNetCut (only 2G)
  ...
  cout << endl;
//
	struct test test_s[BufferNum];
	int eTDDim = 1000;
	alloc_test(test_s, eTDDim);
	for(int i=0; i<BufferNum; i++)
	{
		*(test_s[i].tf) = i;
		cout<<"i = "<<i<<" tf = "<<*(test_s[i].tf)<<endl;
	}
  ...
  if(type==CWB_PLUGIN_ISUPERCLUSTER) {
    ...
    for(int j=0; j<(int)lags; j++) {
      ...
      while(1) {
        ...
        psel += subNetCut(net,(int)j,cfg->subnet,NULL, test_s);
#else
        psel += net->subNetCut((int)j,cfg->subnet,NULL, test_s);
#endif
        bench.Stop();
