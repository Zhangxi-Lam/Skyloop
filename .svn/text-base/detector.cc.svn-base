...
// LCGT interferometer
extern const double _rJ1[3]  = {-3.7685777e6, 3.49218552e6, 3.76722999e6};
extern const double _eJ1x[3] = {-0.5014606643, -0.824380550, 0.262552681483}; 
extern const double _eJ1y[3] = {0.6313793503,-0.1412130545258,0.762508353526};
...
detector::detector(char* name, double t)
{
   ...
   if(strstr(name,"LCGT"))  {pRv=_rJ1; pEx=_eJ1x; pEy=_eJ1y;xifo=true;}
   if(strstr(name,"J1"))    {pRv=_rJ1; pEx=_eJ1x; pEy=_eJ1y;xifo=true;}
...
// get noise rms for specified pixel
	 ...
	 r += 1./x/x;
      ...
      r  = sqrt((m-n)/r);
      if(nVAR.size() && f<nVAR.gethigh() && F>nVAR.getlow()) r *= nVAR.get(t);
      p->setdata(r,'N',I);
...
// returns central frequency of reconstructed waveform
   ...
   return 0.5*F*wf->rate()/E/wf->size();
}
