...
int wat::Time::GpsToGpsLeaps(int gps) { 
  ...
  int i = 1;
  while ((gps >= gps_leaps_table[i].gps) && (i < GPS_LEAPS_TABLE_SIZE)) ++i;
...
int wat::Time::UnixToGpsLeaps(int unix_time) { 
  ...
  int i = 1;
  while ((unix_time >= (gps_leaps_table[i].gps + UTC_UNIX_SPAN - GpsToGpsLeaps(gps_leaps_table[i].gps))) && (i < GPS_LEAPS_TABLE_SIZE)) ++i;
  return gps_leaps_table[i-1].gps_utc;
}
