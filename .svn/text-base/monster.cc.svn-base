...
{  // 
   ...
   for(i=0;i<nRes; ++i){
      ...
      wdm[i] = new WDM<double>(layers[i], wdm0[i]->KWDM, wdm0[i]->BetaOrder, 12);
   }
   ...
   int totOvlps = 0;
   catalog = new   (struct xtalkArray (**[nRes])[2] );
   for(i=0; i<nRes; ++i) catalog[i] = new (struct xtalkArray (*[i+1])[2]);
...
{  //
   ...
   for(int i=0; i<nRes; ++i){
      catalog[i] = new (struct xtalkArray (*[i+1])[2]);
      for(int j=0; j<=i; ++j){
         catalog[i][j] = new struct xtalkArray [layers[i]+1][2];
...
{  //
   ...
   std::vector<wavearray<float> >().swap(clusterCC);
...
//         cout<<(int)pixj->layers<<" ";
         
         tmp.data[n*7+0] = float(m-1);
         tmp.data[n*7+1] = tmpOvlp.CC[0];
         tmp.data[n*7+2] = tmpOvlp.CC[2];
         tmp.data[n*7+3] = tmpOvlp.CC[1];
         tmp.data[n*7+4] = tmpOvlp.CC[3];
         tmp.data[n*7+5] = tmpOvlp.CC[0]*tmpOvlp.CC[0]+tmpOvlp.CC[1]*tmpOvlp.CC[1];
         tmp.data[n*7+6] = tmpOvlp.CC[2]*tmpOvlp.CC[2]+tmpOvlp.CC[3]*tmpOvlp.CC[3];
         n++;
      } 

//      cout<<n<<endl;
      tmp.resize(n*7);
      clusterCC.push_back(tmp);
   }
   return pI;
}
