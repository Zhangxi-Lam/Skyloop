...
template<class DataType_t> 
size_t WSeries<DataType_t>::maxEnergy(wavearray<DataType_t> &ts, Wavelet &w, double dT, int n)
{
...
// param: int       - downsample factor to obtain coarse TD steps
   ...
   size_t K = size_t(ts.rate()*fabs(dT));   // half number of time delays
   size_t k;
   ...
   this->getLayer(xx,-0.1); xx=0;
   this->putLayer(xx,-0.1);
   this->getLayer(xx,this->maxLayer()); xx=0;
   this->putLayer(xx,this->maxLayer());
   this->getLayer(xx,-this->maxLayer()); xx=0;
   this->putLayer(xx,-this->maxLayer());
   return K;
}
...
// create a scan
template<class DataType_t> 
void WSeries<DataType_t>::scan(WSeries<DataType_t> &ws, int n)
{
   int k,m;
   wavearray<DataType_t> x,y,z;

   if(ws.getLevel()) ws.Inverse();
   x = ws;

   if(!this->getLevel()) {                      // initialize scan
      this->setWavelet(*(ws.pWavelet));
      this->Forward(x,n);
      *this = 0.; 
      return;
   }

   int MH = this->getLevel();
   int ML = ws.maxLayer();
   int K = MH/ML;
   WSeries<DataType_t> wH; 
   WSeries<DataType_t> wL;
   wH.setWavelet(*(this->pWavelet));
   wL.setWavelet(*(ws.pWavelet));
   ...
   for(k=0; k<K; k++) {
      ...
      for(m=0; m<ML; m++) {
         wL.getLayer(y,m);
         this->getLayer(z,m*K+k); z+=y;
         this->putLayer(z,m*K+k);
      }
   }
}

// create a scan out of up-sampled TF -series
/*
template<class DataType_t> 
void WSeries<DataType_t>::scan(WSeries<DataType_t> &wH, WSeries<DataType_t> &wL)
{
   int i,j,k,n,m;
   int MH = wH.getLevel();
   int ML = wL.getLevel();
   int nH = wH.size()/(2*MH+2);
   int nL = wL.size()/(2*ML+2);
   int K = MH/ML;
   int K2 = K*2;
   int M2 = MH*2;
   int level = 0;
   
   DataType_t* wH00 = wH.pWavelet->pWWS;
   DataType_t* wH90 = wH.pWavelet->pWWS+nH*(MH+1);
   DataType_t* wL00 = wL.pWavelet->pWWS;
   DataType_t* wL90 = wL.pWavelet->pWWS+nL*(ML+1);
   
   do{level++;} while(M2>>level > 1);
   
   if(nH!=nL || MH%ML || wH.start()!=wL.start() || K<2 || M2!=(1<<level)) {
      printf("WSeries::scan - incompatible TF maps\n");
      return;
   }
   
   *this = (wavearray<DataType_t>)wH;
   this->resize(nH*M2);
   this->setLevel(level);
   this->pWavelet->heterodine(true);
   this->pWavelet->m_TreeType = 1;
   this->pWavelet->rate = M2*wH.pWavelet->rate/(MH+1);
   
   DataType_t* pD = this->data;
   wavearray<double> tmp(ML+1);
   double* norm = tmp.data;
   double a,eL,eH;
   
   // initialize high-F map and produce wavescan
   for(i=0; i<nH; i++){
      k = 0;
      eL = eH = 1.e-200;
      for(j=1; j<=M2; j++){
         m = i*(MH+1)+j/2;
         a = (wH00[m]*wH00[m]+wH90[m]*wH90[m])/2.;
         pD[j-1] = a;
         
         //      eH += a;
         //      eL +=(wL00[n]*wL00[n]+wL90[n]*wL90[n]);
         
         n = (j+K-1)/K2;
         m = i*(ML+1) + n;
         
         eL = (wL00[m]*wL00[m]+wL90[m]*wL90[m]);
         pD[j-1] = a + eL/2.;
         
         if(n==k) {eH += a;} 
         else{
            norm[n] = eH;
            norm[n] = (wL00[m]*wL00[m]+wL90[m]*wL90[m]);
            eH = a+1.e-200; k = n;
         }
      }
      ...
      pD[M2-1] *= 2;
      norm[ML] = 1/eH;
      norm[ML] = (wL00[m]*wL00[m]+wL90[m]*wL90[m]);
      
      for(j=1; j<=M2; j++){
         n = (j+K-1)/K2;
         //      pD[j-1] = sqrt(pD[j-1]*norm[n]);
      }
