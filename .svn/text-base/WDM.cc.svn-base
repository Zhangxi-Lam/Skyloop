...
// flag = -2 indicates that the Quadrature coefficients will be used
  ...
  for(n=0; n<N; ++n){
    ...
    for(j=1; j<M; ++j)
      if( (n+j) & 1 ) imX[j] = sqrt(2)*TFmap[j];
      else  if(j&1) reX[j] = - sqrt(2)*TFmap[j];
      else reX[j] = sqrt(2)*TFmap[j];
...
// inverse transform using the quadrature coefficients
  ...
  for(n=0; n<N; ++n){
    for(j = 0; j<M2; ++j) reX[j] = imX[j] = 0;
    for(j=1; j<M; ++j)
      if( (n+j) & 1 ) reX[j] = sqrt(2)*map90[j];
      else  if(j&1) imX[j] = sqrt(2)*map90[j];
      else imX[j] = -sqrt(2)*map90[j];
