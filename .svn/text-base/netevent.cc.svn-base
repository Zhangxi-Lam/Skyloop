...
void netevent::allocate()
{
   ...
   else              chirp=    (Float_t*)realloc(chirp,5*sizeof(Float_t));
   if (!eBBH)        eBBH=     (Float_t*)malloc(3*sizeof(Float_t));
   else              eBBH=     (Float_t*)realloc(eBBH,2*sizeof(Float_t));
   ...
   if (!Psm)         Psm=      new skymap(int(0));  
   else {delete Psm; Psm=      new skymap(int(0));}
...
// read cluster parameters
   ...
   size_net   = pwc->get((char*)"size",0,'R',0);
   volume_net = pwc->get((char*)"volume",0,'R',0);
   ...
   this->eventID[0] = ID;
   this->eventID[1] = 0;
   this->iota[0] = pcd->iota;
   this->psi[0] = pcd->polarisation;
   ...
   this->anet = pcd->aNET;
   this->inet = pcd->iNET;
   this->norm = pcd->skyStat;                       // localization statistic 
   this->likelihood = pcd->likenet;
   this->size[0] = int(size_net.data[kid]+0.5);     // event size
   this->size[1] = pcd->skySize;                    // event size in skyloop
   this->chirp[1] = pcd->mchirp;                    // reconstructed chirp mass
   this->chirp[2] = pcd->mchirperr;                 // reconstructed chirp mass error
   this->chirp[3] = pcd->tmrgr;                     // reconstructed chirp time
   this->chirp[4] = pcd->tmrgrerr;                  // reconstructed chirp time error
   this->chirp[5] = pcd->chi2chirp;                 // chi2chirp
   ...
   for(i=0; i<int(nIFO); i++) {                         // loop on detectors
      ...
      this->volume[i] =       int(volume_net.data[kid]+0.5);

      this->snr[i]  =         pd->enrg;
      this->nill[i] =         pd->xSNR-pd->sSNR;
      this->null[i] =         pd->null+2.*this->size[0]/nIFO;  // null per detector
      this->xSNR[i] =         pd->xSNR;
      this->sSNR[i] =         pd->sSNR;
      this->neted[0] =        pcd->isoED;                  // network ED with isolation correction
      this->neted[1] =        pcd->netED;                  // network ED
      this->neted[2] +=       fabs(pd->xSNR-pd->sSNR);     // standard network ED
      this->neted[3] +=       this->null[i];               // total null energy in time domain
      this->neted[4]  =       pcd->energy;                 // total energy in allresolutions
      ...
      double xstop  = this->gps[i]+this->left[i]+this->right[i]+this->duration[i]-net->Edge;  // end data
      this->time[i] += lag[i];
      ...
      this->hrss[i] =         sqrt(pd->get_SS()/16384.);
      this->noise[i] =        pow(10.,noise_net.data[m])/sqrt(16384.);
      this->bp[i]=            Aa.real();
      this->bx[i]=            Aa.imag();
      ...
      x = 1. - pd->sSNR/(this->snr[i]+this->size[0]);
   ...
   this->ECOR = this->ecor;
   this->netcc[0] = pcd->isocc;                           // cc with isolation correction  (CBC search) 
   this->netcc[1] = pcd->skycc;                           // second cc statistic (all-sky search)
   this->netcc[2] = pcd->SUBNET;                          // second suBNetCut statistic
   this->netcc[3] = pcd->subnet;                          // sub-network statistic 
   this->rho[0]   = pcd->netRHO;                          // coherent SNR per detector
   this->rho[1]   = pcd->netrho;                          // reduced coherent SNR per detector
	 ...
	 for(i=0; i<N; i++)                              // loop on detectors
	 {
	    ...
	    this->bandwidth[i] =    high_net.data[k] - low_net.data[k];
	      ...
	      this->hrss[i] =       pow(pow(10.,hrss_net.data[m]),2)+pow(pow(10.,hrss_NET.data[m]),2);
              this->hrss[i] =       sqrt(this->hrss[i]/16384.);
            } else {   
              this->hrss[i] =       pow(10.,hrss_net.data[m])/sqrt(16384.);
            }
	    this->noise[i] =        pow(10.,noise_net.data[m])/sqrt(16384.);
	    this->bp[i]=            Aa.real();
	    this->bx[i]=            Aa.imag();
