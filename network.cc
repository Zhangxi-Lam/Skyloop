...
//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	...
	FILE *fpt = fopen("./new_debug/k4_bb", "a");
	FILE *fpt1 = fopen("./new_debug/k4_BB", "a");
  ...
  skyloop:
	...
	//fprintf(fpt, "k = %d l = %d bb[0] = %f BB[0] = %f\n", k, l, a_00.data[0], a_90.data[0]);
...
/*	if(k==4)
	{
		float tmp[4];
		_mm_storeu_ps(tmp, *(_pp+jf));
		fprintf(fpt, "k = %d l = %d bb[%d] = %f bb[%d] = %f bb[%d] = %f bb[%d] = %f\n", k, l, j*NIFO, tmp[0], j*NIFO+1, tmp[1], j*NIFO+2, tmp[2], j*NIFO+3, tmp[3]);
		_mm_storeu_ps(tmp, *(_PP+jf));
		fprintf(fpt1, "k = %d l = %d BB[%d] = %f BB[%d] = %f BB[%d] = %f BB[%d] = %f\n", k, l, j*NIFO, tmp[0], j*NIFO+1, tmp[1], j*NIFO+2, tmp[2], j*NIFO+3, tmp[3]);
	}*/
	    if(ee<En) continue;
...
/*	if(k==4)
	{
		fprintf(fpt, "k = 4 l = %d ee[0] = %f ee[1] = %f ee[2] = %f\n", l, ee_a[0], ee_a[1], ee_a[2]);
		fprintf(fpt1, "k = 4 l = %d em[0] = %f em[1] = %f em[2] = %f\n", l, em_a[0], em_a[1], em_a[2]);
	}*/
		
	//fprintf(fpt, "k = %d l = %d ee = %f em = %f Ls = %f Eo = %f Ln = %f m = %d\n", k, l, ee, em, Ls, Eo, Ln, m);
	/*if(k==4&&l==0)
	{
		for(int i=0; i<16; i++)
			fprintf(fpt, "k = %d l = %d Fp = %f Fx = %f\n", k, l, Fp.data[i], Fx.data[i]);
	}*/
	...
	{
		int i;
		fprintf(fpt[0], "k = %d l = %d m = %d fp[0] = %f fp[1] = %f fp[2] = %f fp[3] = %f\n", k, l, m4, fp.data[0], fp.data[1], fp.data[2], fp.data[3]);
		fprintf(fpt[1], "k = %d l = %d m = %d fp[0] = %f fp[1] = %f fp[2] = %f fp[3] = %f\n", k, l, m4, fp.data[4], fp.data[5], fp.data[6], fp.data[7]);
		fprintf(fpt[2], "k = %d l = %d m = %d fp[0] = %f fp[1] = %f fp[2] = %f fp[3] = %f\n", k, l, m4, fp.data[8], fp.data[9], fp.data[10], fp.data[11]);
		fprintf(fpt[3], "k = %d l = %d m = %d fp[0] = %f fp[1] = %f fp[2] = %f fp[3] = %f\n", k, l, m4, fp.data[12], fp.data[13], fp.data[14], fp.data[15]);

		fprintf(fpt[4], "k = %d l = %d m = %d fx[0] = %f fx[1] = %f fx[2] = %f fx[3] = %f\n", k, l, m4, fx.data[0], fx.data[1], fx.data[2], fx.data[3]);
		fprintf(fpt[5], "k = %d l = %d m = %d fx[0] = %f fx[1] = %f fx[2] = %f fx[3] = %f\n", k, l, m4, fx.data[4], fx.data[5], fx.data[6], fx.data[7]);
		fprintf(fpt[6], "k = %d l = %d m = %d fx[0] = %f fx[1] = %f fx[2] = %f fx[3] = %f\n", k, l, m4, fx.data[8], fx.data[9], fx.data[10], fx.data[11]);
		fprintf(fpt[7], "k = %d l = %d m = %d fx[0] = %f fx[1] = %f fx[2] = %f fx[3] = %f\n", k, l, m4, fx.data[12], fx.data[13], fx.data[14], fx.data[15]);
	}*/

         for(j=0; j<m4; j+=4) {                                   
            int jf = j*f_;
	    //_sse_dpf4_ps(_Fp+jf,_Fx+jf,_fp+jf,_fx+jf, k, l, fpt);                // go to DPF
	    _sse_dpf4_ps(_Fp+jf,_Fx+jf,_fp+jf,_fx+jf);                // go to DPF
/*	if(k==4)
	{
		float tmp[4];
		_mm_storeu_ps(tmp, *(_fp+jf));
		fprintf(fpt[0], "k = %d l = %d fp[0] = %f fp[1] = %f fp[2] = %f fp[3] = %f\n", k, l, tmp[0], tmp[1], tmp[2], tmp[3]);
		_mm_storeu_ps(tmp, *(_fx+jf));
		fprintf(fpt[1], "k = %d l = %d fx[0] = %f fx[1] = %f fx[2] = %f fx[3] = %f\n", k, l, tmp[0], tmp[1], tmp[2], tmp[3]);
	}*/
/*	if(k==4)
	{
		for(int i=0; i<16; i++)
...
//	if(k==4)
//		fprintf(fpt1, "k = %d l = %d stat = %f Lm = %f Em = %f Am = %f lm = %d Vm = %d suball = %f EE = %f\n", k, l, AA, Lo, Eo, aa, l, m, ee, em);
...
//		fprintf(fpt, "stat = %f Lm = %f Em = %f Am = %f lm = %f Vm = %f suball = %f EE = %f Eo = %f Lo = %f Ls = %f\n", stat, Lm, Em, lm, Vm, suball, EE, Eo, Lo, Ls);
...
//    fclose(fpt);
	...
	if (k==4)
	 ...
	 rHo = sqrt(Lo*Lo/(Eo+2*m)/nIFO);                            // estimator of coherent amplitude
      ...
      if(hist && rHo>this->netRHO) 
	 for(j=0;j<vint->size();j++) hist->Fill(suball,submra);

      if(fmin(suball,submra)>TH && rHo>this->netRHO) {
         count += vint->size();	
	 if(hist) {
