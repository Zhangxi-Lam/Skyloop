...
extern void after_skyloop(void *post_gpu_data, network *net, TH2F *hist, netcluster *pwc, double **FP, double **FX, float **pa, float **pA, int pixelcount, int Lsky, double *gpu_time, size_t *streamCount, int streamNum, float &Lo);
...
//++++++++++++++++++++++++++++++++
	...
	v_size = vDim * gpu_nIFO * 2 + HEAD_SIZE + nr_size;
	...
	cudaMemcpyToSymbol(constLmax, &lmax, sizeof(int));
		...
		if( (v_ptr+2*gpu_nIFO*vtddim+nr_size)<=v_size && ((pixelCount+1)*OutputSize+alloced_V+V)<=(OutputSize+Vmax) )
		{
			...
			for(j=0; j<V; j++)
			{
				...
				//for(i=0; i<nIFO; i++)
					...
					{
						pre_gpu_data[alloced_gpu].other_data.vtd_vTD_nr[v_ptr + vtddim*gpu_nIFO*2 + j*gpu_nIFO+i] = 0;
	                                	for( l=0; l<tsize; l++)
	                                       	{
...
__host__ void push_work_into_gpu(struct pre_data *input_data, struct post_data *post_gpu_data, struct skyloop_output *skyloop_output, struct other *skyloop_other, size_t *vtddim_array, size_t *etddim_array, size_t *alloced_V_array, int Lsky, int *pixel_array, int work_size, cudaStream_t *stream)
{
        ...
        for(int i=0; i<work_size; i++)// call for gpu caculation
	{
		...
		kernel_reduction<<<pixel_array[i], num_threads, shared_memory_usage, stream[i]>>>(skyloop_other[i].tmp, skyloop_output[i].output, skyloop_other[i].eTD, skyloop_other[i].vtd_vTD_nr, skyloop_other[0].ml_mm, skyloop_other[0].V_tsize);
		kernel_clear<<<num_blocks, num_threads, shared_memory_usage, stream[i]>>>(skyloop_other[i].tmp);	// clear the tmp memory
	}
...
__global__ void kernel_skyloop(float *eTD, float *vtd_vTD_nr, double *FP_FX, short *ml_mm, size_t *V_tsize, float *gpu_BB, float *gpu_bb, float *gpu_fp, float *gpu_fx, float *gpu_Fp, float *gpu_Fx, float *gpu_tmp, float *gpu_output, int pixelcount)
{
	...
	size_t etd_ptr = 0;
	size_t output_ptr = 0;
	int reduct_tid = constLmax % GRID_SIZE;
...
/*			if(k==4)
			...
			gpu_output[count*OutputSize] = etd_ptr;
		...
		gpu_tmp[3*GRID_SIZE + tid + GRID_SIZE*OutputSize*count] = _stat.Am;
		gpu_tmp[4*GRID_SIZE + tid + GRID_SIZE*OutputSize*count] = _stat.suball;
		gpu_tmp[5*GRID_SIZE + tid + GRID_SIZE*OutputSize*count] = _stat.EE;
...
/*	if(k==2 && l == 196607)
	{
		gpu_output[0] = threadIdx.x + blockDim.x*blockIdx.x;
		gpu_output[1] = _s->Eo;
		gpu_output[2] = _s->Ls;
		gpu_output[3] = _s->m;
	}*/
...
/*	if(k==4)
	{
		int Lsky = 196608;
		gpu_output[l] = _En[0];
		gpu_output[l+Lsky] = _En[1];
		gpu_output[l+2*Lsky] = _En[2];
		gpu_output[l+3*Lsky] = _En[3];
	}*/
	...
	/*if(k == 353)
	{
		int Lsky = 196608;
		gpu_output[l] = aa;
		gpu_output[l+Lsky] = Eo;
		gpu_output[l+2*Lsky] = Lo;
		gpu_output[l+3*Lsky] = m;
		gpu_output[l+4*Lsky] = Ln;
	}*/	
...
/*		if(blockIdx.x == 1)
		...
		// Get goto rNRG
		lm = tmp[l + 6*GRID_SIZE + blockIdx.x*GRID_SIZE*OutputSize];
		if(lm != -1)
		{
			count = 0;
			k_array = vtd_vTD_nr;
			ml[0] = ml_mm;
			ml[1] = ml_mm + constLsky;
			ml[2] = ml_mm + 2*constLsky;
			while( count<blockIdx.x )
			{
				k = k_array[count] - 1;
				V = V_tsize[k];
				tsize = V_tsize[k+constK];
				etd_ptr += V*tsize*gpu_nIFO;
				count++;
			}
			k = k_array[count] - 1;
			V = V_tsize[k];
			...
			pe[1] = eTD + vDim + etd_ptr + (tsize/2)*V;
			pe[2] = eTD + 2*vDim + etd_ptr + (tsize/2)*V;
                	pe[0] = pe[0] + ml[0][lm] * (int)V;
	                pe[1] = pe[1] + ml[1][lm] * (int)V;
        	        pe[2] = pe[2] + ml[2][lm] * (int)V;
			for(int v=0; v<V; v++)
			{
				gpu_output[v + MaxPixel*OutputSize + blockIdx.x*VMAX]= pe[0][v] + pe[1][v] + pe[2][v];
			}
		}		
...
{
	int tid = blockIdx.x * blockDim.x + threadIdx.x;
	size_t Dim = GRID_SIZE*OutputSize*MaxPixel;
...
/*	if(k==4)
	{
		int Lsky = 196608;
		gpu_output[l] = *(fp);
		gpu_output[l+Lsky] = *(fp+GRID_SIZE);
		gpu_output[l+2*Lsky] = *(fp+2*GRID_SIZE);
		gpu_output[l+3*Lsky] = *(fp+3*GRID_SIZE);
		gpu_output[l+4*Lsky] = *(fx);
		gpu_output[l+5*Lsky] = *(fx+GRID_SIZE);
		gpu_output[l+6*Lsky] = *(fx+2*GRID_SIZE);
		gpu_output[l+7*Lsky] = *(fx+3*GRID_SIZE);
	}*/
...
__inline__ __device__ void kernel_sse_rot4p_m_ps(float *Fp, float *_c, float *Fx, float *_s, float *fp, float *fx)
{
	...
	fp[NIFO*GRID_SIZE] = Fp[NIFO*GRID_SIZE]*_c[1] + Fx[NIFO*GRID_SIZE]*_s[1];	
	fx[NIFO*GRID_SIZE] = Fx[NIFO*GRID_SIZE]*_c[1] - Fp[NIFO*GRID_SIZE]*_s[1];	
	fp[NIFO*GRID_SIZE+GRID_SIZE] = Fp[NIFO*GRID_SIZE+GRID_SIZE]*_c[1] + Fx[NIFO*GRID_SIZE+GRID_SIZE]*_s[1];	
	fx[NIFO*GRID_SIZE+GRID_SIZE] = Fx[NIFO*GRID_SIZE+GRID_SIZE]*_c[1] - Fp[NIFO*GRID_SIZE+GRID_SIZE]*_s[1];	
	fp[NIFO*GRID_SIZE+2*GRID_SIZE] = Fp[NIFO*GRID_SIZE+2*GRID_SIZE]*_c[1] + Fx[NIFO*GRID_SIZE+2*GRID_SIZE]*_s[1];	
	fx[NIFO*GRID_SIZE+2*GRID_SIZE] = Fx[NIFO*GRID_SIZE+2*GRID_SIZE]*_c[1] - Fp[NIFO*GRID_SIZE+2*GRID_SIZE]*_s[1];	

	fp[2*NIFO*GRID_SIZE] = Fp[2*NIFO*GRID_SIZE]*_c[2] + Fx[2*NIFO*GRID_SIZE]*_s[2];	
	fx[2*NIFO*GRID_SIZE] = Fx[2*NIFO*GRID_SIZE]*_c[2] - Fp[2*NIFO*GRID_SIZE]*_s[2];	
	fp[2*NIFO*GRID_SIZE+GRID_SIZE] = Fp[2*NIFO*GRID_SIZE+GRID_SIZE]*_c[2] + Fx[2*NIFO*GRID_SIZE+GRID_SIZE]*_s[2];	
	fx[2*NIFO*GRID_SIZE+GRID_SIZE] = Fx[2*NIFO*GRID_SIZE+GRID_SIZE]*_c[2] - Fp[2*NIFO*GRID_SIZE+GRID_SIZE]*_s[2];	
	fp[2*NIFO*GRID_SIZE+2*GRID_SIZE] = Fp[2*NIFO*GRID_SIZE+2*GRID_SIZE]*_c[2] + Fx[2*NIFO*GRID_SIZE+2*GRID_SIZE]*_s[2];	
	fx[2*NIFO*GRID_SIZE+2*GRID_SIZE] = Fx[2*NIFO*GRID_SIZE+2*GRID_SIZE]*_c[2] - Fp[2*NIFO*GRID_SIZE+2*GRID_SIZE]*_s[2];	

	fp[3*NIFO*GRID_SIZE] = Fp[3*NIFO*GRID_SIZE]*_c[3] + Fx[3*NIFO*GRID_SIZE]*_s[3];	
	fx[3*NIFO*GRID_SIZE] = Fx[3*NIFO*GRID_SIZE]*_c[3] - Fp[3*NIFO*GRID_SIZE]*_s[3];	
	fp[3*NIFO*GRID_SIZE+GRID_SIZE] = Fp[3*NIFO*GRID_SIZE+GRID_SIZE]*_c[3] + Fx[3*NIFO*GRID_SIZE+GRID_SIZE]*_s[3];	
	fx[3*NIFO*GRID_SIZE+GRID_SIZE] = Fx[3*NIFO*GRID_SIZE+GRID_SIZE]*_c[3] - Fp[3*NIFO*GRID_SIZE+GRID_SIZE]*_s[3];	
	fp[3*NIFO*GRID_SIZE+2*GRID_SIZE] = Fp[3*NIFO*GRID_SIZE+2*GRID_SIZE]*_c[3] + Fx[3*NIFO*GRID_SIZE+2*GRID_SIZE]*_s[3];	
	fx[3*NIFO*GRID_SIZE+2*GRID_SIZE] = Fx[3*NIFO*GRID_SIZE+2*GRID_SIZE]*_c[3] - Fp[3*NIFO*GRID_SIZE+2*GRID_SIZE]*_s[3];	
...
void allocate_cpu_mem(struct pre_data *pre_gpu_data, struct post_data *post_gpu_data, int vDim, int Vmax, int Lsky, size_t K)// allocate locked memory on CPU 
{
        ...
        for(int i = 0; i<StreamNum; i++)
        {
//                CUDA_CHECK(cudaMalloc(&(skyloop_output[i].output), MaxPixel*OutputSize*sizeof(float) + MaxPixel*VMAX*sizeof(float) ) );
...
void allocate_gpu_mem(struct skyloop_output *skyloop_output, struct other *skyloop_other, int vDim, int Vmax, int Lsky, size_t K)// allocate the memory on GPU
{
        for(int i = 0; i<StreamNum; i++)
        {
                ...
                CUDA_CHECK(cudaMalloc(&(skyloop_other[i].Fp), num_blocks * num_threads * Vmax * NIFO * sizeof(float) ) );
                CUDA_CHECK(cudaMalloc(&(skyloop_other[i].Fx), num_blocks * num_threads * Vmax * NIFO * sizeof(float) ) );
                CUDA_CHECK(cudaMalloc(&(skyloop_other[i].tmp), num_blocks * num_threads * OutputSize * MaxPixel * sizeof(float) ) );
//                CUDA_CHECK(cudaMalloc(&(skyloop_output[i].output), MaxPixel*OutputSize*sizeof(float) + MaxPixel*VMAX*sizeof(float) ) );
