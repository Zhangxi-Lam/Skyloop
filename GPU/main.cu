...
//++++++++++++++++++++++++++++++++
	...
	for(k=0; k<K; k++)				// loop over clusters
	{
		V_array[k] = 0;
		tsize_array[k] = 0;
		pre_gpu_data[0].other_data.V_tsize[k] = 0;
		pre_gpu_data[0].other_data.V_tsize[k+K] = 0;
		...
		kcount++;
		pre_gpu_data[0].other_data.V_tsize[k] = V;
		pre_gpu_data[0].other_data.V_tsize[k+K] = tsize;
...
__host__ void push_work_into_gpu(struct pre_data *input_data, struct post_data *post_gpu_data, struct skyloop_output *skyloop_output, struct other *skyloop_other, size_t *alloced_V4_array, size_t *etddim_array, size_t k_array[][MaxPixel], int Lsky, int *pixel_array, int work_size, cudaStream_t *stream)
{
	...
	for(int i=0; i<work_size; i++)// call for gpu caculation
		kernel_skyloop<<<num_blocks, num_threads, shared_memory_usage, stream[i]>>>(skyloop_other[i].eTD, skyloop_other[0].ml_mm, skyloop_other[0].V_tsize, skyloop_output[i].output, Lsky);
...
__global__ void kernel_skyloop(float *eTD, short *ml_mm, size_t *V_tsize, float *gpu_output, int Lsky, size_t K) 
{
