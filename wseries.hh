...
class WSeries : public wavearray<DataType_t>
{
   ...
   public:
	 ...
	 return pWavelet->BinaryTree() ? (1<<getLevel())-1 : getLevel();
      ...
      inline double resolution(size_t n=0){
	 ...
	 return (n<S.size()) ? this->data[n*S.stride()+S.start()] : 0; 
      ...
      // !param: n - wavelet time index
      // !param: m - layer number
      inline DataType_t putSample(DataType_t a, int n, double m) {
         std::slice S = this->getSlice(m);
	 if(n<S.size()) this->data[n*S.stride()+S.start()]=a; 
      ...
      size_t maxEnergy(wavearray<DataType_t>&ts, Wavelet &w, double t, int n=1);

      // produce wavescan from two up-sampled WDM TF data
      // par1 - high-frequency resolution WDM 
      // par2 - low-frequency resolution WDM 
      void scan(WSeries<DataType_t> &, int=-1);
      //      void scan(WSeries<DataType_t> &, WSeries<DataType_t> &);
